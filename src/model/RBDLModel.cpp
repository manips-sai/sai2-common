...
RBDLModel::RBDLModel (const std::string path_to_model_file, Model::ParserType parser, bool verbose)
{
    // parse model from file with the right parser
    switch(parser)
    {
	case urdf :
	{
	    // parse rbdl model from urdf
	    bool success = RigidBodyDynamics::Addons::URDFReadFromFile(path_to_model_file.c_str(), &_rbdl_model, false, verbose);
	    if (!success) {
