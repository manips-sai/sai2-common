...
static void loadLinkCollision(cDynamicLink* link, const my_shared_ptr<sai_urdf::Collision>& collision_ptr, const std::string& working_dirname = "./") {
	...
	if (geom_type == sai_urdf::Geometry::MESH) {
		...
		const auto mesh_ptr = dynamic_cast<const sai_urdf::Mesh*>(collision_ptr->geometry.get());
		assert(mesh_ptr);
		// load object
		...
		const auto box_ptr = dynamic_cast<const sai_urdf::Box*>(collision_ptr->geometry.get());
		assert(box_ptr);
		...
		const auto sphere_ptr = dynamic_cast<const sai_urdf::Sphere*>(collision_ptr->geometry.get());
		assert(sphere_ptr);
		...
		const auto cylinder_ptr = dynamic_cast<const sai_urdf::Cylinder*>(collision_ptr->geometry.get());
		assert(cylinder_ptr);
		// create chai sphere mesh
...
static void loadLinkInertial(cDynamicLink* link, ConstLinkPtr& urdf_link) {
	...
	if (urdf_link->inertial) {
		...
		// TODO: abort if link inertia is not diagonal
...
void URDFToDynamics3dWorld(const std::string& filename, cDynamicWorld* world, bool verbose) {
	...
	for (const auto object_pair: urdf_world->graphics_.static_objects) {
		...
		link->m_name = "static_object_link";
...
void URDFToDynamics3dRobot(const std::string& filename, cDynamicBase* model, bool verbose, const std::string& working_dirname) {
	...
	for (unsigned int j = 0; j < joint_names.size(); j++) {
		...
		switch (urdf_joint_type) {
			...
			case sai_urdf::Joint::FLOATING:
			case sai_urdf::Joint::PLANAR:
			case sai_urdf::Joint::CONTINUOUS:
				cerr << "Unsupported joint type on joint " << joint_names[j] << endl;
				abort();
				break;
			case sai_urdf::Joint::FIXED: //This is true for ground links
			default:
				break;
		}
		if (NULL != dyn_joint) {
